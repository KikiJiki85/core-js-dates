...
function getTime(date) {
  const seconds = date.getSeconds(date).toString().padStart(2, '0');
  const minutes = date.getMinutes(date).toString().padStart(2, '0');
  const hours = date.getHours(date).toString().padStart(2, '0');
...
function getDayName(date) {
  ...
  return weekDays[dateObject.getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  const weekDay = newDate.getUTCDay();
  let nextFriday = (5 - weekDay + 7) % 7;
  if (nextFriday === 0) nextFriday = 7;
  newDate.setUTCDate(newDate.getUTCDate() + nextFriday);
  return newDate;
}
